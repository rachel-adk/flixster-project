  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (response.ok) {
          setMovies(
            data.results.filter(({ poster_path }) => poster_path != null)
          );
          setError(null);
        } else {
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.themoviedb.org/3/movie/now_playing?api_key=${apiKey}&language=en-US&page=${nextPage}`
      );
      const data = await response.json();
        ...
        <div className="movieList">
          {movies.map((movie, index) => (
            <MovieCard
              ...
              hasWatched={
                watchedMovies.filter((m) => m.id === movie.id).length > 0
              }
